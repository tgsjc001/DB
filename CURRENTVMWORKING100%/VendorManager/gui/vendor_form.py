...
class VendorForm(tk.Toplevel):
    ...
    def build_form(self):
        ...
        def label_entry(row, label, key, width=18):
            ...
            entry = tk.Entry(wrapper, font=("Roboto", 14), width=width, bg="#2e2e2e", fg="white",
                             ...
                             disabledbackground="#2e2e2e", textvariable=var)
            ...
            elif key in ("reservation_date", "first_show", "last_show_attended"):
                def format_date(*_):
            ...
            # Insert value if editing an existing vendor
            formatted_value = ""
            if self.vendor and hasattr(self.vendor, key):
        ...
        label_entry(row0, "Show Date", "show_date")
        ...
        tk.Label(row11, text="Total Due", bg="#1e1e1e", fg="#ffdf00", font=("Roboto", 14)).grid(row=0, column=0, padx=5, pady=4, sticky="ew")
        ...
        tk.Label(row11, text="Amount Paid", bg="#1e1e1e", fg="#ffdf00", font=("Roboto", 14)).grid(row=0, column=2, padx=5, pady=4, sticky="ew")
        ...
        tk.Label(row11, text="Receipt #", bg="#1e1e1e", fg="#ffdf00", font=("Roboto", 14)).grid(row=0, column=4, padx=5, pady=4, sticky="ew")
        ...
        tk.Label(row11, text="Tax ID", bg="#1e1e1e", fg="#ffdf00", font=("Roboto", 14)).grid(row=0, column=8, padx=5, pady=4, sticky="ew")
    ...
    def save_vendor(self):
        ...
        for k, w in self.entries.items():
            ...
            if k in ("reservation_date", "first_show", "last_show_attended"):
                val = None if val in ("", "--/--/--") else val

            # Validate state
            if k == "state":
                if not re.fullmatch(r"[A-Z]{2}", val.upper()):
        ...
        try:
            log_audit(
                self.user.username,
                "Vendor Save",
                f"Vendor {vendor_name} saved successfully."
            )
        except Exception as e:
        ...
        try:
            log_activity(
                ...
                self.user.username,
                "Vendor Save",
                f"Updated vendor record: {vendor_name}"
            )
        except Exception as e:
            print("[LOG ACTIVITY ERROR]", e)

        # Avoid session conflict by merging
    ...
    def load_vendor_data(self):
        for k, w in self.entries.items():
            if hasattr(self.vendor, k):
                ...
                if isinstance(w, tk.Text):
                    ...
                    if k in ("total_due", "amount_paid", "taca_amount"):
                        w.delete(0, tk.END)
        ...
        self.on_rollover_var.set(self.vendor.on_rollover)
