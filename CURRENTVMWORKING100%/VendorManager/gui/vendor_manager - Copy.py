...
class VendorManagerWindow(tk.Frame):
	def __init__(self, master, user):
		...
		# Dashboard Tab (Tab 0)
		dashboard_tab = tk.Frame(self.notebook, bg="#1e1e1e")
		self.notebook.add(dashboard_tab, text="Dashboard")

		# Tile-style buttons for navigation
		buttons = [
			("Vendors", lambda: self.notebook.select(1)),
			("Mailouts", lambda: self.notebook.select(2)),
			("Logs", lambda: self.notebook.select(3))
		]
		for i, (label, cmd) in enumerate(buttons):
			btn = tk.Button(dashboard_tab, text=label, command=cmd,
							bg="#2e2e2e", fg="white", width=15, height=6)
			btn.grid(row=i // 3, column=i % 3, padx=20, pady=20)

		self.build_layout()
		self.build_mailout_tab()
		# Logs Tab (moved to end)
		logs_tab = tk.Frame(self.notebook, bg="#1e1e1e")
		self.notebook.add(logs_tab, text="Logs")
		logs_notebook = ttk.Notebook(logs_tab)
		logs_notebook.pack(fill='both', expand=True)

		# Audit Logs sub-tab
		frame_audit = tk.Frame(logs_notebook, bg="#1e1e1e")
		logs_notebook.add(frame_audit, text="Audit Logs")
		audit_tree = ttk.Treeview(frame_audit, columns=("timestamp", "username", "action", "description"),
								  show="headings")
		for col in ("timestamp", "username", "action", "description"):
			audit_tree.heading(col, text=col.title(), command=lambda c=col, t=audit_tree: self.sort_tree(t, c, False))
			audit_tree.column(col, width=150)
		audit_tree.pack(fill="both", expand=True)

		# Activity Logs sub-tab
		frame_activity = tk.Frame(logs_notebook, bg="#1e1e1e")
		logs_notebook.add(frame_activity, text="Activity Logs")
		activity_tree = ttk.Treeview(frame_activity, columns=("timestamp", "username", "action", "description"),
									 show="headings")
		for col in ("timestamp", "username", "action", "description"):
			activity_tree.heading(col, text=col.title(),
								  command=lambda c=col, t=activity_tree: self.sort_tree(t, c, False))
			activity_tree.column(col, width=150)
		activity_tree.pack(fill="both", expand=True)

		# Session Logs sub-tab
		frame_session = tk.Frame(logs_notebook, bg="#1e1e1e")
		logs_notebook.add(frame_session, text="Session Logs")
		session_tree = ttk.Treeview(frame_session, columns=("login_time", "logout_time", "username", "ip_address"),
									show="headings")
		for col in ("login_time", "logout_time", "username", "ip_address"):
			session_tree.heading(col, text=col.title(),
								 command=lambda c=col, t=session_tree: self.sort_tree(t, c, False))
			session_tree.column(col, width=150)
		session_tree.pack(fill="both", expand=True)

		def on_logs_tab_change(event):
			selected = logs_notebook.index(logs_notebook.select())
			if selected == 0:
				self.load_audit_logs(audit_tree)
			elif selected == 1:
				self.load_activity_logs(activity_tree)
			elif selected == 2:
				self.load_session_logs(session_tree)

		logs_notebook.bind("<<NotebookTabChanged>>", on_logs_tab_change)
	...
	def build_layout(self):
		vendor_tab = tk.Frame(self.notebook, bg="#1e1e1e")
		self.notebook.add(vendor_tab, text="Vendors")
		self.vendor_tab = vendor_tab
		self.build_vendor_list_ui()

	def build_vendor_list_ui(self):
		print("[DEBUG] build_layout starting")

		control_frame = tk.Frame(self.vendor_tab, bg="#1e1e1e")
		...
		export_frame = tk.Frame(self.vendor_tab, bg="#1e1e1e")
		...
		self.tree = ttk.Treeview(self.vendor_tab,
	...
	def build_legend_status_bar(self):
		...
		"""
		legend_frame = tk.Frame(self.vendor_tab, bg="#2e2e2e")
	...
	def build_mailout_tab(self):
		mailout_tab = tk.Frame(self.notebook, bg="#1e1e1e")
		self.notebook.add(mailout_tab, text="Mailouts")
		MailoutManagerWindow(mailout_tab, self.user).pack(fill="both", expand=True)
	...
	def open_mailout_manager(self):
		mailout_window = tk.Toplevel(self)
		mailout_window.title("Mailout Manager")
		mailout_window.geometry("1000x700")
		MailoutManagerWindow(mailout_window, self.user).pack(fill="both", expand=True)
...
def set_double_booked_flags(vendors):
	...
	def load_audit_logs(self, tree):
		session = Session()
		tree.delete(*tree.get_children())
		for log in session.query(AuditLog).filter_by(visible=True).all():
			tree.insert("", "end", values=(log.timestamp, log.username, log.action, log.description))
		session.close()

	def load_activity_logs(self, tree):
		session = Session()
		tree.delete(*tree.get_children())
		for log in session.query(ActivityLog).filter_by(visible=True).all():
			tree.insert("", "end", values=(log.timestamp, log.username, log.action, log.description))
		session.close()

	def load_session_logs(self, tree):
		session = Session()
		tree.delete(*tree.get_children())
		for log in session.query(SessionLog).filter_by(visible=True).all():
			tree.insert("", "end", values=(log.login_time, log.logout_time, log.username, log.ip_address))
		session.close()
